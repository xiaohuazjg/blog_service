...
import (
	"github.com/xiaohuazjg/blog_service/internal/dao"
...
func (svc *Service) GetArticleList(param *ArticleListRequest, pager *app.Pager) ([]*Article, int, error) {
	...
	articles, err := svc.dao.GetArticleTagByTagID(param.TagID, param.State, pager.Page, pager.PageSize)
	if err != nil {
		return nil, 0, err
	}
	...
	for _, article := range articles {
		articlelist = append(articlelist, &Article{
			...
			CoverImageUrl: article.CoverImageUrl,
			State:         article.State,
			Tag:           &model.Tag{Model: &modle.Model{ID: article.TagID}, Name: article.TagName},
		})
	}
