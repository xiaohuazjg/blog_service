...
func (a ArticleTag) GetByAID(db *gorm.DB) (ArticleTag, error) {
	...
	err := db.Where("article_id=? AND id_del=?", a.ArticleID, 0).First(&articleTag).Error
...
func (a ArticleTag) ListByTID(db *gorm.DB) ([]*ArticleTag, error) {
	...
	if err := db.Where("tag_id=? AND id_del=?", a.TagID, 0).Find(&articleTags).Error; err != nil {
		return nil, err
	}
...
func (a ArticleTag) listByAID(db *gorm.DB, articleIDs []uint32) ([]*ArticleTag, error) {
	var articleTags []*ArticleTag
