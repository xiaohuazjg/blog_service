...
func (a Article) Update(db *gorm.DB, values interface{}) error {
	if err := db.Model(&a).Where("id=? AND is_del=?", a.ID, 0).Update(values).Error; er != nil {
		return err
	}
	return nil
}
...
func (a Article) Get(db *gorm.DB) (*Article, error) {
	var article Article
	db = db.Model(&a).Where("id=? AND state=? AND is_del=?", a.ID, a.State, 0)
...
func (a Article) ListByTagID(db *gorm.DB, TagID uint32, pageOffset, pageSize int) ([]*ArticleRow, error) {
	...
	rows, err := db.Select(fields).table(Article{}.TableName()+"AS at").
	...
	for rows.Next() {
		r := &ArticleRow{}
		if err := rows.Scan(&r.ArticleID, &r.ArticleTitle, &r.ArticleDesc, &r.ArticleImageUrl, &r.Content, &r.TagID, &r.TagName); err != nil {
			return nil, err
		}
